...
from models.mymodels import ResNet18Based, VGG16Based
...
dataset = pd.read_csv('datasets/csv.csv')
...
class Dataset:
    ...
    def __getitem__(self, i):
        img = Image.open('datasets/' + self.imgs[i]).convert('RGB')
        img = np.array(img)
...
def val(model, data_val, loss_function, writer, epoch, device):
    ...
    f1 = F1Score(num_classes=14, task='multiclass').to(device)
    total_loss = 0
    total_samples = 0
...
def main():
    ...
    model = ResNet18Based().to(device)   # will be VGG18Base and ResNet18Base seperately
    optimizer = Adam(model.parameters(), lr=5e-5) # will be 0.00005, 0.001 and 0.1
